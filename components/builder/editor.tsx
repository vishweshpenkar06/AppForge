...
export default function BuilderEditor() {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
      <section className="lg:col-span-7 rounded-2xl border border-white/8 bg-white/[0.02] p-4">
        ...
        <textarea
          ...
          className="w-full min-h-[320px] resize-y rounded-lg bg-transparent p-3 text-sm leading-6 outline-none border border-white/6"
        />
      </section>

      <aside className="lg:col-span-5 rounded-2xl border border-white/8 bg-white/[0.02] p-4">
        <div className="flex items-center justify-between mb-3">
        ...
        <div className="space-y-2 max-h-[480px] overflow-auto">
          {history.length === 0 && <p className="text-xs text-zinc-500">No saved prompts yet.</p>}
          {history.map((h) => (
            <div key={h.id} className="rounded-lg border border-white/6 p-2">
              <div className="flex items-start justify-between gap-2">
                <div className="text-xs text-zinc-300">{new Date(h.when).toLocaleString()}</div>
              ...
              <p className="mt-2 text-sm text-zinc-400 line-clamp-3">{h.text}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-4">
          ...
          {!result && <p className="text-xs text-zinc-500">No compile run yet.</p>}
          {result && (
            <div className="mt-2 text-xs text-zinc-300">
              ...
              {result.error && (
                <div className="mb-2 text-rose-400">Error: {String(result.error)}</div>
              )}
              {result.docs && (
                <div className="mb-2">
              ...
              {result.downloadUrl && (
                <div className="mt-2">
                  <a href={result.downloadUrl} target="_blank" rel="noreferrer" className="text-sky-400 underline">Download generated package</a>
                </div>
              )}
